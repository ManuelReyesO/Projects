...
namespace PuntoLaLuz
{
    public partial class Cajas : Form
    {
        ...
        private void radProductos(object sender, EventArgs e)
        {
            ...
            {
                //Buscando producto seleccionado, guardando nombre y precio
                prod = productos.Text;
        ...
        {
            //Condicionales para extras de la barra (Para seleccionar mas de 1 extra)
            ...
            if (prod != "")
            {
                //Añadiendo los productos seleccionados en las listas
                Producto.Add(prod);
                Tipo.Add(type);
                Extra.Add(ex);
                Precio.Add(cashE + cashP + cashT);

                //Proyectando el contenido de las listas con un datasource
                L_Producto.DataSource = null;
                L_Tipo.DataSource = null;
                L_Extra.DataSource = null;
                L_Precio.DataSource = null;
                ...
                //Calculo para el total de la compra
                txt_total.Text = Precio.Sum().ToString();
            }
            else
            {
                MessageBox.Show("Debe escojer un prodúcto", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        ...
        {
            //Validación en caso de no seleccionar nada
            if (txt_total.Text != "") 
            {
                //Insertando los datos de la compra en la base de datos
                ...
                MessageBox.Show("Compra realizada con exito.\nRetire su comprobante de compra.", "Compra", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Producto.Clear();
                Tipo.Clear();
                Extra.Clear();
                Precio.Clear();
        ...
        //Deshace el proceso de compra actual
